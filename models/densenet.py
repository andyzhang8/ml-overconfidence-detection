...
class DenseNetTabular(nn.Module):
    def __init__(self, input_dim, num_classes, growth_rate=64, num_blocks=3, num_layers_per_block=2):
        super(DenseNetTabular, self).__init__()
        self.initial_layer = nn.Linear(input_dim, growth_rate)

        self.blocks = nn.Sequential(
            *[DenseBlock(growth_rate * (i + 1), growth_rate, num_layers_per_block) for i in range(num_blocks)]
        )

        final_dim = growth_rate * (num_blocks + 1)
        self.output_layer = nn.Linear(final_dim, num_classes)
